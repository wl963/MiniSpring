...
public class RequestMappingHandlerMapping implements HandlerMapping{
            ...
            if(methods!=null) {
                for (Method method : methods) {
                    ...
                    if(annotationPresent)
                    {
                        ...
                        mappingRegistry.getMappingMethodNames().put(url,methodName);
                        mappingRegistry.getMappingClasses().put(url,clz);
                    }
                }
            }
        }
    ...
    public HandlerMethod getHandler(HttpServletRequest request) throws Exception {
        ...
        Class<?> aClass = mappingRegistry.getMappingClasses().get(servletPath);
        String methodName = mappingRegistry.getMappingMethodNames().get(servletPath);

        HandlerMethod handlerMethod = new HandlerMethod(method, obj,aClass,methodName);
